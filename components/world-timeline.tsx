...
export function WorldTimelineControls({status,progress,disabled,onSeek}:{status:TimelineStatus;progress:SeekProgress|null;disabled:boolean;onSeek:(month:number|null)=>void}){
 ...
 const [scheduler]=useState(()=>createSeekScheduler(onSeek));
 useEffect(()=>{scheduler.setHandler(onSeek);},[scheduler,onSeek]);
 useEffect(()=>{scheduler.invalidate();},[scheduler,status.latestMonth,status.earliestMonth]);
 ...
 const jump=(month:number|null)=>{setDraft(null);scheduler.commit(month);};
 const move=(month:number)=>{setDraft(String(month));scheduler.schedule(month);};
 return <section className={`history-timeline ${status.historical?'is-historical':''}`} aria-label="历史时间轴">
  ...
  <div className="history-slider"><Button variant="ghost" size="icon-sm" aria-label="查看上一个月" disabled={disabled||status.viewMonth<=status.earliestMonth} onClick={()=>jump(status.viewMonth-1)}><ArrowLeft size={16}/></Button><Slider aria-label="选择已模拟的月份" value={[valid?value:status.viewMonth]} min={status.earliestMonth} max={Math.max(status.earliestMonth+1,status.latestMonth)} step={1} disabled={disabled||status.latestMonth===status.earliestMonth} onValueChange={v=>move(Array.isArray(v)?v[0]:v)} onValueCommitted={v=>jump(Array.isArray(v)?v[0]:v)}/><Button variant="ghost" size="icon-sm" aria-label="查看下一个月" disabled={disabled||status.viewMonth>=status.latestMonth} onClick={()=>jump(status.viewMonth+1)}><ArrowRight size={16}/></Button></div>
